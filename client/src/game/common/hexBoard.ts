...
import {HexUtils, Node} from './hexLibraries/hexUtils';
...
export class HexBoard {
  ...
  xyToHexIndex(x: number, y: number): GridHexagon {
  ...
  pathFind(start: GridHexagon, finish: GridHexagon) {
    ...
    let closed: Node<GridHexagon>[] = [];
    const result: GridHexagon[] = [];
    let neighbours;
    let node;
    let path;
    let length: number;
    ...
    while ((length = open.length)) {
      ...
      if (node.x === myPathEnd.x && node.y === myPathEnd.y) {
        ...
        for (i = 0, j = neighbours.length; i < j; i++) {
          ...
          if (!n) continue;
          if (Math.abs(node.item!.y + node.item!.height - (n.y + n.height)) >= 2) continue;
          path = new Node(node, n);
          if (!aStar[path.value()]) {
            path.g =
              node.g +
              HexUtils.distance(n, node.item!) +
              Math.abs(node.item!.y + node.item!.height - (n.y + n.height)) * 2;
